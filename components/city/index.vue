  ...
  <view>
    <view class="header w100 flexw" :style="{ height: customBar }">
      ...
      <view class="flex w100">
        <input
          ...
          @input="onInput"
          placeholder="搜索城市 中文/拼音/首字母"
          v-model="searchValue"
        />
        <view class="back_div fsmall bold blue" @tap="back()"> 取消 </view>
      </view>
    </view>
    ...
    >
      <view v-if="disdingwei" id="hot">
        <!-- 定位模块 -->
        <view class="dingwei">
          <view class="grey flexa fsmall"> 当前定位 </view>
        ...
        <view class="dingwei" v-if="cacheLocation">
          <view class="grey flexa fsmall"> 最近访问 </view>
          <view class="dingwei_city dingwei_city_zuijin">
            <view
              class="dingwei_city_one"
              v-for="(item, index) in cacheLocation"
              :key="index"
              @tap="back_city(item)"
            >
              {{ item.city_name }}
        ...
        <view class="dingwei" v-if="hotCity">
          <view class="grey flexa fsmall"> 热门城市 </view>
          <view class="dingwei_city dingwei_city_zuijin">
            <view
              class="dingwei_city_one"
      ...
      <view v-if="searchValue == ''">
        <block v-for="(item, index) in list" :id="item.idx" :key="index">
...
<script>
import Citys from "@/components/city/acity.js";
export default {
  data() {
    return {
      statusBar: "0px",
